...
This runs `docker compose up -d --build`, waits for **`GET /health`**, runs **`npm run smoke:webhooks`**, then **`npm run verify:lead-capture-states`** (Website **none / partial / captured** on unified pipeline), then **`docker compose down`**.  
...
| **T1 — default local** | After Docker or infra change | **`npm run staging:docker-smoke`** (compose **without** `.env`) | **`[smoke] all passed`** on **seven** webhook routes **and** **`[lead-verify] all passed`** |
