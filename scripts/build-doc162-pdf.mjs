#!/usr/bin/env node
/**
 * Build printable HTML + PDF for docs/162 (Markdown → marked → wrapped HTML → Edge headless print-to-pdf).
...
const mdName = '162_customer_seven_channel_access_token_guide.md';
const mdPath = path.join(root, 'docs', mdName);
const printHtmlPath = path.join(root, 'docs', '162_customer_seven_channel_access_token_guide_print.html');
const pdfPath = path.join(root, 'docs', '162_customer_seven_channel_access_token_guide.pdf');
...
<title>ChatFlow Pro — 七通道凭据获取说明（客户版）</title>
<style>
...
if (!fs.existsSync(pdfPath)) {
  console.error('PDF was not created. Open docs/162_customer_seven_channel_access_token_guide_print.html and print to PDF manually.');
  process.exit(1);
}
