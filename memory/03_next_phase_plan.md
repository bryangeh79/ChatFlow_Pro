...
- Next Unique Priority Action: 生产按 **`docs/161`** 对接（含新增 request_id 字段）；新客户阅读 **`docs/162`**（PDF）；可选 **`dev:notify-echo`** 本地验 POST；按需 **notify / suppress / assign-strategy / agent-status / assignment-audit / assignment-report / sla-report / daily-trend / ops-alerts / autotune**；**docs/157** 有 staging URL 再跑全量；门禁 **T0+T1**；**`docs/155`** *T1 equivalence* 适用于无 Docker 的代理环境。
