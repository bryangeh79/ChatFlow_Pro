...
- **Phase 21 ADR（先文档后代码）**: **`docs/165_phase21_handoff_autotune_config_reload_admin_api_adr.md`** — autotune 后续落地优先 **白名单 handoff 调参 JSON + 进程内重读**（SIGHUP/定时）；可选未来 **最小 admin HTTP**；默认生产仍以部署/重启与 advisory autotune 为准。
- Next Unique Priority Action: 生产按 **`docs/161`** 对接（含新增 request_id 字段）；新客户阅读 **`docs/162`**（PDF）；可选 **`dev:notify-echo`** 本地验 POST；按需 **notify / suppress / assign-strategy / agent-status / assignment-audit / assignment-report / sla-report / daily-trend / ops-alerts / autotune**；**docs/157** 有 staging URL 再跑全量；下一产品块 **Phase 21**（把建议真正落地为 env reload 或 admin API）；门禁 **T0+T1**；**`docs/155`** *T1 equivalence* 适用于无 Docker 的代理环境。
