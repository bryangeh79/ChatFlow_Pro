...
- **版本**：`package.json` **1.7.80**（**Pro_v1.07.80**）— Phase 24：**Auth-RBAC Foundation（1A–1J）checkpoint 已封**；**包 2A–2E ✅**（**2E** = **`postgres/*.sql` DDL 资产** + **registry checksum 绑定**，仍 **无执行**、无 `pg`）；**下一**：**真实 Postgres apply** + **ledger 落库** + **CI** — 见 **`docs/177`**。SaaS MVP 仍 **sealed**。  
...
- **当前版本（package.json）**：**1.7.80**（**Pro_v1.07.80**）— Phase 24 **2E 已交付**（SQL 静态资产 + checksum）；**推进中：Postgres 真执行 / 落库 / CI**。SaaS MVP 封板语义不变。
...
2. **Postgres + migration**（**2A–2E ✅（含 SQL 资产+checksum）→ 真 pg/执行/落库/CI**；local 可保留 sql.js，hosted 目标 Postgres）。
