...
- **版本**：`package.json` **1.7.86**（**Pro_v1.07.86**）— Phase 24：**Auth-RBAC Foundation（1A–1J）checkpoint 已封**；**包 2A–2K ✅**（**2K** = **`postgres-config` DSN/字段契约 + readiness 字段**；**无** TCP/probe/pool；**2J** loader、**2I** gate）；**下一**：**2L connect probe**、**pool + query**、**migration/ledger 真执行** — 见 **`docs/177`**、**`docs/178`**。SaaS MVP 仍 **sealed**。  
...
- **当前版本（package.json）**：**1.7.86**（**Pro_v1.07.86**）— Phase 24 **2K 已交付**（connection config contract）；**推进中：2L probe / runtime / DB ledger**。SaaS MVP 封板语义不变。
