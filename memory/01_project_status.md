...
- Current Version: **Pro_v1.07.44** (package.json: 1.7.44)
...
- Next Unique Priority Action: 生产按 **`docs/161`** 对接（幂等 + 验 secret）；本地可用 **`npm run dev:notify-echo`**；按需配置 **notify / suppress**；日常 **T0+T1**；**docs/157** 有 staging URL 再跑完整验证；下一工程包候选：**notify JSON 带 `request_id` 对齐 `X-Request-Id`**；产品大块：**memory/32** 或坐席分配（改共享管道时对照 **04/06/07**）
