...
- Current Version: **Pro_v1.07.31** (package.json: 1.7.31)
...
- Current Completion Point: **Pro_v1.07.31** — **`.env` 策略**：**`docs/155`** *Environment & secrets*；**`.env.example` 纳入 git**、**`.gitignore`** 误忽略已修；**AGENTS** 红线；**verify:lead-capture**（Website+TG）；**CI v5** + **`report:github-ci`**；**160** §4 / §4.5
