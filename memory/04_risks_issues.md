...
- **版本 / Phase**：**1.7.78**；**Phase 24** — **Auth-RBAC Foundation（1A–1J）checkpoint 已封**；**包 2A/2B/2C ✅**（2C = Postgres **stub** + driver 选择，**默认 sqljs**）；**下一 2C+** 真 pg / 迁移；**Phase 23 / SaaS MVP 主线已关闭**。  
...
- **Postgres adapter（2C）**：**仅为 stub** — 设 `CHATFLOW_SAAS_DB_DRIVER=postgres` 时 **任何 DB 调用即抛** `postgres_adapter_not_implemented`；**未**接 `pg`、**无** migration、**无** CI Postgres runtime — **勿当可跑生产后端**。  
