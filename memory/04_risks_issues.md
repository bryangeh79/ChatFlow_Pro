...
- **版本 / Phase**：**1.7.74**；**Phase 24** 当前（**1H** token storage hardening），**Phase 23 / SaaS MVP 主线已关闭**。  
...
- **包 1H**：**新写入** principal **不再明文落库**（`bridge_token_hash` + 占位 `bridge_token`）；**历史行**可经首次鉴权 **懒迁移**；**仍无** password、JWT、session、**登录审计**、KMS、**rotation 产品**、加盐 — **非**终态凭证方案；env / break-glass 仍属运维明文面。  
